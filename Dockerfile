...
# ---------------------------------------------------------------------------
# SCAIL kpopshorts additions — mirror of Josh's local env (ComfyUI 0.20.3)
# ---------------------------------------------------------------------------
RUN cd /comfyui/custom_nodes  && git clone https://github.com/Kosinkadink/ComfyUI-VideoHelperSuite  && git -C ComfyUI-VideoHelperSuite checkout 993082e4f247  && git clone https://github.com/kijai/ComfyUI-KJNodes  && git -C ComfyUI-KJNodes checkout fca78c93f034  && git clone https://github.com/pythongosssss/ComfyUI-Custom-Scripts  && git -C ComfyUI-Custom-Scripts checkout f2838ed5e59d  && git clone --depth 1 https://github.com/pythongosssss/ComfyUI-WD14-Tagger  && git clone --depth 1 https://github.com/rgthree/rgthree-comfy

RUN set -e; for f in /comfyui/custom_nodes/*/requirements.txt; do uv pip install -r "$f"; done

COPY custom/ComfyUI-SCAIL2-ColoredMask /comfyui/custom_nodes/ComfyUI-SCAIL2-ColoredMask

# WD14 eva02 tagger model baked in, sha-pinned to Josh's local copy
RUN mkdir -p /comfyui/custom_nodes/ComfyUI-WD14-Tagger/models  && wget -q -O /comfyui/custom_nodes/ComfyUI-WD14-Tagger/models/wd-eva02-large-tagger-v3.onnx       https://huggingface.co/SmilingWolf/wd-eva02-large-tagger-v3/resolve/main/model.onnx  && echo "9e768793060c7939b277ccb382783e8670e8a042d29d77aa736be0c8cc898bfc  /comfyui/custom_nodes/ComfyUI-WD14-Tagger/models/wd-eva02-large-tagger-v3.onnx" | sha256sum -c -
COPY custom/wd-eva02-large-tagger-v3.csv /comfyui/custom_nodes/ComfyUI-WD14-Tagger/models/wd-eva02-large-tagger-v3.csv
